...
def calculate_checkpoint_bonus(path, checkpoints):
    ...
    return (
        len(visited_checkpoints) * 50
    )  # Adjust the weight based on the importance of checkpoints
...
def calculate_diversity_penalty(path, previous_paths):
    ...
    similarity_scores = [
        calculate_similarity(path, prev_path) for prev_path in previous_paths
    ]
...
def fitness_function(path, checkpoints, generation):
    ...
    revisit_penalty = -len(
        set(path)
    )  # Penalize paths that visit the same location multiple times
    ...
    if generation < 5:
        diversity_penalty = calculate_diversity_penalty(
            path, previous_paths[:generation]
        )
