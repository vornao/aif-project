...
def define_reward(monster: str = "kobold"):
    ...
    reward_manager.add_eat_event(
        name="apple", reward=2, terminal_sufficient=True, terminal_required=True
    )
...
def perform_action(action, env):
    ...
    obs, reward, done, info = env.step(
        action_id
    )  # N.B. env.step() dice di compiere quell'azione, non per forza c'entra con i passi
    ...
    for i in range(21):
        for j in range(79):
            if not (obs["screen_descriptions"][i][j] == 0).all():
                obj = (
                    bytes(obs["screen_descriptions"][i][j])
                    .decode("utf-8")
                    .rstrip("\x00")
                )
                if "apple" in obj:
